...
def main():
    ...
    if configs.load_model_path != "None":
        if not configs.use_lora:
            ...
            model = PeftModel.from_pretrained(model,configs.load_model_path)
            loaded=True
    ...
    if configs.use_lora and not loaded:
        lora_config = LoraConfig(
            r=configs.lora_r,                          # Rank of LoRA matrices
            lora_alpha=configs.lora_alpha,                # Scaling factor
            lora_dropout=configs.lora_dropout,            # Dropout probability
            target_modules=configs.lora_target_modules,  # for GPT-2 (adjust based on model architecture)
            task_type="CAUSAL_LM"
        )
    ...
    if configs.only_eval:
        ...
        parallel_model = FSDP(
            model, auto_wrap_policy=llama_auto_wrap_policy, device_id=rank, use_orig_params=True if configs.use_lora else False
        )
        ...
        if not configs.only_eval:
            ...
            if (
                ...
                if not configs.use_lora:
                    ...
                    # Consolidate state dict at rank 0
                    with FSDP.state_dict_type(
                        parallel_model, StateDictType.FULL_STATE_DICT, state_dict_config
                    ):
                        state_dict = parallel_model.state_dict()
                    if rank == 0:
                        cpu_model = AutoModelForCausalLM.from_pretrained(configs.model_id)
                        cpu_model = get_peft_model(cpu_model, lora_config)
                        # Load consolidated weights
                        cpu_model.load_state_dict(state_dict, strict=False)

                        # Now save LoRA adapters (rank 0 only)
                        cpu_model.save_pretrained(os.path.join(save_dir, f"checkpoint_{epoch + 1}"))
                    del state_dict
                print("saving model.")
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(valid_gen_dataloader):
                ...
                if not configs.use_lora:
                    ...
                    model_for_gen = parallel_model                        # FSDP wrapper
                    with FSDP.summon_full_params(model_for_gen, recurse=False):
                        # inside the context all FlatParameters are gathered → real tensors (2‑D)
                        inner = model_for_gen.module                      # unwrap FSDP once
                        if isinstance(inner, PeftModel):                  # unwrap PEFT if present
                            inner = inner.base_model.model                # → HF base model

                        outputs = inner.generate(
                            **batch,
                            max_new_tokens=max_new_tokens,
                            synced_gpus= not configs.only_eval      # generating inside summon_full_params ⇒ keep False
                        )
                ...
                if idx < 5 and rank == 0:
                    # print some examples
                    print(
                        f"Question {question[0]}: Answer = '{answer[0]}' CoT = '{answer_cot[0]}'"
                    )
                    print(f"Full output: '{text_output[0]}'")
                    print(f"Extracted Output: '{answer_output[0]}'")
